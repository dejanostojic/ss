...
var menu = {
	init: function() {
		...
		$('.mixitup').mixItUp({
			...
			load: {
				filter: (document.location.hash === '' || document.location.hash === null) ? '.menu-meals' : ('.' + document.location.hash.substring(1))
			},
			callbacks: {
				...
				onMixStart: function(state, futureState) {//alert('onmixstart current: ' + document.location.hash); alert('onmixstart future: ' + futureState.activeFilter)
					document.location.hash = futureState.activeFilter.substring(1);
				},
				onMixLoad: function(state, futureState) {
					that.lazyLoad();
					if (document.location.hash !== '#menu-meals') {
						if($('.section-intro').length > 0)
