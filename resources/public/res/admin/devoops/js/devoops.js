...
function LoadLeafletScript(callback){
	if (!$.fn.L){
		$.getScript('/res/admin/devoops/plugins/leaflet/leaflet.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadTimePickerScript(callback){
	if (!$.fn.timepicker){
		$.getScript('/res/admin/devoops/plugins/jquery-ui-timepicker-addon/jquery-ui-timepicker-addon.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadBootstrapValidatorScript(callback){
	if (!$.fn.bootstrapValidator){
		$.getScript('/res/admin/devoops/plugins/bootstrapvalidator/bootstrapValidator.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadSelect2Script(callback){
	if (!$.fn.select2){
		$.getScript('/res/admin/devoops/plugins/select2/select2.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadDataTablesScripts(callback){
	function LoadDatatables(){
		$.getScript('/res/admin/devoops/plugins/datatables/jquery.dataTables.js', function(){
			$.getScript('/res/admin/devoops/plugins/datatables/ZeroClipboard.js', function(){
				$.getScript('/res/admin/devoops/plugins/datatables/TableTools.js', function(){
					$.getScript('/res/admin/devoops/plugins/datatables/dataTables.bootstrap.js', callback);
				});
			});
		});
	}
...
function LoadFineUploader(callback){
	if (!$.fn.fineuploader){
		$.getScript('/res/admin/devoops/plugins/fineuploader/jquery.fineuploader-5.0.5.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadXChartScript(callback){
	function LoadXChart(){
		$.getScript('/res/admin/devoops/plugins/xcharts/xcharts.min.js', callback);
	}
	function LoadD3Script(){
		if (!$.fn.d3){
			$.getScript('/res/admin/devoops/plugins/d3/d3.min.js', LoadXChart)
		}
		else {
			LoadXChart();
		}
...
function LoadFlotScripts(callback){
	function LoadFlotScript(){
		$.getScript('/res/admin/devoops/plugins/flot/jquery.flot.js', LoadFlotResizeScript);
	}
	function LoadFlotResizeScript(){
		$.getScript('/res/admin/devoops/plugins/flot/jquery.flot.resize.js', LoadFlotTimeScript);
	}
	function LoadFlotTimeScript(){
		$.getScript('/res/admin/devoops/plugins/flot/jquery.flot.time.js', callback);
	}
	if (!$.fn.flot){
		LoadFlotScript();
	}
...
function LoadMorrisScripts(callback){
	function LoadMorrisScript(){
		if(!$.fn.Morris){
			$.getScript('/res/admin/devoops/plugins/morris/morris.min.js', callback);
		}
		else {
			if (callback && typeof(callback) === "function") {
				callback();
	...
	if (!$.fn.raphael){
		$.getScript('/res/admin/devoops/plugins/raphael/raphael-min.js', LoadMorrisScript);
	}
	else {
		LoadMorrisScript();
	}
...
function LoadAmchartsScripts(callback){
	function LoadAmchartsScript(){
		$.getScript('/res/admin/devoops/plugins/amcharts/amcharts.js', LoadFunnelScript);
	}
	function LoadFunnelScript(){
		$.getScript('/res/admin/devoops/plugins/amcharts/funnel.js', LoadSerialScript);
	}
	function LoadSerialScript(){
		$.getScript('/res/admin/devoops/plugins/amcharts/serial.js', LoadPieScript);
	}
	function LoadPieScript(){
		$.getScript('/res/admin/devoops/plugins/amcharts/pie.js', callback);
	}
	if (!$.fn.AmCharts){
		LoadAmchartsScript();
	}
...
function LoadChartistScripts(callback){
	function LoadChartistScript(){
		$.getScript('/res/admin/devoops/plugins/chartist/chartist.min.js', callback);
	}
	if (!$.fn.Chartist){
		LoadChartistScript();
	}
...
function LoadSpringyScripts(callback){
	function LoadSpringyScript(){
		$.getScript('/res/admin/devoops/plugins/springy/springy.js', LoadSpringyUIScript);
	}
	function LoadSpringyUIScript(){
		$.getScript('/res/admin/devoops/plugins/springy/springyui.js', callback);
	}
	if (!$.fn.Springy){
		LoadSpringyScript();
	}
...
function LoadFancyboxScript(callback){
	if (!$.fn.fancybox){
		$.getScript('/res/admin/devoops/plugins/fancybox/jquery.fancybox.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadKnobScripts(callback){
	if(!$.fn.knob){
		$.getScript('/res/admin/devoops/plugins/jQuery-Knob/jquery.knob.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadSparkLineScript(callback){
	if(!$.fn.sparkline){
		$.getScript('/res/admin/devoops/plugins/sparkline/jquery.sparkline.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function TinyMCEStart(elem, mode){
	...
	console.log('TinyMCEStart: ' + elem  + ", mode: " + mode + tinymce);
}
//
...
function DrawCalendar(){
	...
	$('#external-events div.external-event').each(function() {
		...
		var $this = $(this);
		var eventObject = {
			title: $this.attr('data-title'), // use the element's text as the event title
	...
	function addProductInMenu(timeInMilis,productId, successCallback){
	    $.ajax({
            type: "POST",
            url: "/admin/shop/menu/calendar",
            data: {"date": timeInMilis, "product_id": productId},
	...
	var calendar = $('#calendar').fullCalendar({
		...
		eventSources: [
                { url: '/admin/shop/menu/calendar',
                    ...
                    console.log("data: " + JSON.stringify({oldVal: eventData,
                                                          title: eventData.shop_product.name,
                                                          description: eventData.shop_product.ingredients,
                                                          productId: eventData.product_id
                                                          }));*/

                    return {oldVal: eventData,
                            title: eventData.shop_product.name,
                            id: eventData.id,
                            description: eventData.shop_product.ingredients,
                            productId: eventData.product_id,
                            start: moment(eventData.date, moment.ISO_8601),
                            allDay: true,
                            shop_product_type: eventData.shop_product.shop_product_type.id,
                            backgroundColor: eventData.shop_product.shop_product_type.color || 'black'
                            };
                }},
                { url: '/admin/shop/menu/activeWeek', // FIX TO MARK ACTIVE WEEK
					eventDataTransform: function( eventData ) {

						return {oldVal: eventData,
								title: '',
								start: eventData['start-date'],
								className: ['calendar-week-highlight'],
								allDay: true,
								editable: false,
								backgroundColor: '#FAE6EE',
								borderColor: '#FAE6EE',
								textColor: '#FAE6EE'
								};
					},
		...
		eventAfterAllRender: function (view) { // FIX TO MARK ACTIVE WEEK
			$('.fc-day.fc-widget-content').closest('.fc-week').find('.calendar-week-highlight').closest('.fc-week').addClass('calendar-week-highlight');
		},

		eventOrder: 'shop_product_type',
		selectable: true,
		selectHelper: true,
		weekNumbers:true,
/*		select: function(start, end, allDay) {
            ...
            BootstrapDialog.show({
                ...
                message: "OVO ĆU POSLE DA SREDIM. Za sad prevlači.",
                buttons: [{
...
// stara forma
		...
		*/
		...
		drop: function(date, allDay) { // this function is called when something is dropped
			...
			console.log('prod_id: ' + copiedEventObject.productId);
            addProductInMenu(date.valueOf(), copiedEventObject.productId, function(data, textStatus, jqXHR){
		...
		eventClick: function(calEvent, jsEvent, view) {
			...
			$('#event_delete').on('click', function(){
                $.ajax({type: "DELETE",
                        url: "/admin/shop/menu/calendar/" + calEvent._id,
                        cache: false,
                        success: function( data, textStatus, jqXHR ){
...
$(document).ready(function () {
	...
	function handleMenuToggling(e){
		var $anchor = $(this);
		var parents = $anchor.parents('li');
		var li = $anchor.closest('li.dropdown');
		var another_items = $('.main-menu li').not(parents);
		another_items.find('a').removeClass('active');
		another_items.find('a').removeClass('active-parent');
		if ($anchor.hasClass('dropdown-toggle') || $anchor.closest('li').find('ul').length == 0) {
			$anchor.addClass('active-parent');
			...
			if (li.find('a.dropdown-toggle').hasClass('active-parent')) {
				var pre = $anchor.closest('ul.dropdown-menu');
				pre.find("li.dropdown").not($(this).closest('li')).find('ul.dropdown-menu').slideUp('fast');
			}
		}
		if ($anchor.hasClass('active') == false) {
			$anchor.parents("ul.dropdown-menu").find('a').removeClass('active');
...
/*	
 * 
 * 		HISTORY DISABLED> TINYMCE BUG!
 * 
	// initialize history api
	Dost.history.init({});
	// bind history to all links that have class ajax-link and provide callback function
//	Dost.history.attach($('a.ajax-link'),{},function($elem){
//		$elem.addClass('active');
//		console.log('as param');
//	});
		...
		*/
	Dost.history.init({});
	Dost.history.attach2({
					'jQuerySelector': 'a.ajax-link',
					'options': null, // not mandatory
					'ajaxFn': function(state){
						console.log('ajax fn!!!')
						var animationName = 'animated bounce',
							animationEnd = 'webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend';
						
						// remove editor
						var editor = $('textarea.prog-body-mce').first().tinymce();
						...
						$.ajax({
							...
							success: function(data) {
								...
								$(state.target).html(data).addClass(animationName).one(animationEnd, function(){
									$(state.target).removeClass(animationName);
									console.log('end animation show preloader');
								});
	...
	$('.main-menu').on('click', 'a', handleMenuToggling);
	...
	$('#ajax-content').on('submit', 'form[method="PUT"]', function(e){
	    e.preventDefault();
	    if (tinyMCE){
	    	tinyMCE.triggerSave();
	    }
    ...
    $('#ajax-content').on('submit', 'form[method="POST"]', function(e){
	    ...
	    console.log('req made');
	    console.log('action: ' + action);
	    console.log('data: ' + serializedForm);
	    $.ajax({
	    	type: 'POST',
	    	url: action,
	    	data: serializedForm,
